...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(`https://v3.football.api-sports.io/fixtures?date=${today}`, {
    headers: { 'x-apisports-key': 'ae7d4e153f29e96d0bb7f5e498fb8f25' }
  })
  .then(res => res.json())
  .then(data => {
    ...
    data.response.forEach(match => {
      ...
      card.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px;">
          <img src="${homeLogo}" alt="${home}" width="30" height="30" onerror="this.src='default-team.png'">
          <strong>${home}</strong>
          <span style="margin: 0 5px;">${scoreHome}‑${scoreAway}</span>
          <strong>${away}</strong>
          <img src="${awayLogo}" alt="${away}" width="30" height="30" onerror="this.src='default-team.png'">
        </div>
        <p>${league} • ${status}</p>`;
  ...
  .then(data => {
    ...
    data.results.slice(0, 6).forEach(article => {
      ...
      card.innerHTML = `<h3>${title}</h3><p>${source} • ${date}</p>`;
